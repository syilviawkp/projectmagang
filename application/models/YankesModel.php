...
class YankesModel extends CI_Model {
    ...
    public function update($id, $idField){
        ...
        $this->db->update('detaillaporan', $object);
           ...
           $query = $this->db->query('SELECT * FROM formatkategori WHERE jenisLaporan= "YANKES" ');
            foreach ($query->result() as $key) {
...
public function getFilterKategori(){
   ...
   $query2 = $this->db->query("SELECT * from laporan WHERE jenisLaporan ='YANKES' ORDER by kodeLaporan  DESC LIMIT 1
");
   $bulan = "Januari";
   $tahun = "2019";
